...
class TCTColBERT():
    ...
    def retrieve(self, 
            ...
            with Run().context(RunConfig(nranks=1, experiment="default")):
                ...
                self.indexer.index(name="wikiqa", collection=corpus_texts, overwrite="reuse")

                self.indexer.get_index()
                searcher = Searcher(index="wikiqa", collection=corpus_texts)

                result_qrels = {}
                queries_new = {}
                for query in queries:
                    queries_new[query.id()] = query.text()
                rankings = searcher.search_all(queries_new,k=top_k).todict()
                for idx,query in tqdm.tqdm(enumerate(queries)):
                    result_qrels[str(query.id())] = {}
                    # self.indexer.index(name="wikiqa", collection=corpus_texts, overwrite=True)

                    # self.indexer.get_index()
                    # searcher = Searcher(index="wikiqa", collection=corpus_texts)
                    #results = searcher.search(query.text(), k=top_k)
                #print(rankings)
                for index, ranking in enumerate(list(rankings.keys())):
                    for passage_id, passage_rank, passage_score in rankings[ranking]:
                        if str(passage_id)==str(queries[index].id()):
                            continue
                        result_qrels[str(queries[index].id())][str(corpus[passage_id].id())] = passage_score
                #results = searcher.search_all(queries_text,k=100)
            return result_qrels