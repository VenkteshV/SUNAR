...
class HfRetriever(BaseRetriver):
    ...
    def __init__(self,config=DenseHyperParams) -> None:
       ...
       # self.context_tokenizer = AutoTokenizer.from_pretrained(self.config.document_encoder_path)
        self.question_encoder = AutoModel.from_pretrained(self.config.query_encoder_path, device_map="auto")
    ...
    def encode_corpus(self, 
        ...
        with torch.no_grad():
            while index < len(contexts):
                ...
                tokenized_contexts = self.question_tokenizer(samples, padding=True, truncation=True, return_tensors='pt').to("cuda")
                token_emb =  self.question_encoder(**tokenized_contexts)
